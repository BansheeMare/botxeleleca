import nest_asyncio, asyncio, os, requests, queue, json, re
...
IP_DA_API = 'http://54.255.240.75/'
URL_REDIRECT = 'http://54.255.240.75/callback'
...
TOKEN_RETRY = 104
# Caminho do arquivo JSON para armazenar as configurações de recuperação
RECUPERACAO_CONFIG_FILE = 'configuracoes_recuperacao.json' 
# Caminho do arquivo JSON para armazenar os id dos GRUPOS
GRUPO_ID_FILE = 'grupo_id.json' 
...
async def disparar_recuperacao(chat_id, payment_id):
    ...
    try:
        continue_recovery = cache['payments'][payment_id]['continue']
        for config in configuracoes_recuperacao:
            if continue_recovery:    
                # Configuração do botão "Pagar Agora"
                keyboard = [[InlineKeyboardButton("Pagar Agora", callback_data=f"pagar_{payment_id}")]]
...
def add_payment_info(id, chat, recovery):
    rec = load_recuperacao_config(cache['bot_id'])
    payment = {'chat':chat,'tempo':rec[0]['tempo'] * 60, "recovery":recovery, 'continue':True}
    payment['tempo'] = 30
    cache['payments'][id] = payment
        ...
        if len(payment_cache) > 0:
            ...
            for payment_id in payment_cache:
                if payment_id in cache['payments'].keys():
                    ...
                    if not payment_data:
                        pass
                        #remove_payment_info(payment_id)
                    else:
                        if payment_data[4] != 'paid':
                            if cache['payments'][payment_id]['tempo'] < 5:
                                expire_payment_info(payment_id)
                                if cache['payments'][payment_id]['recovery']:
                                    await disparar_recuperacao(user_chat, payment_id)
                                    cache['payments'][payment_id]['recovery'] = False
                            else:
                                cache['payments'][payment_id]['tempo'] = cache['payments'][payment_id]['tempo'] - 2
                        else:
...
async def recebe_texto_recuperacao(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💲 Envie o desconto que quer aplicar na recuperação de compra (em %):")
...
async def recebe_desconto_recuperacao(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❗️ Por favor, envie um desconto válido entre 0 e 100.")
        return INPUT_DESCONTO_RECUPERACAO
...
async def receber_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not (user_link.startswith("http://") or user_link.startswith("https://") or user_link.startswith("t.me/")):
...
async def recebe_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
...
def save_upsell(content, bot_id):
    manager.update_bot_upcell(bot_id, content)
...
async def recebe_link_upsell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Validação do link
        if not (upsell_link.startswith("http://") or upsell_link.startswith("https://") or upsell_link.startswith("t.me/")):
            await update.message.reply_text(
                "⚠️ O link enviado é inválido. O link deve começar com 'http://', 'https://', ou 't.me/'."
            )
        ...
        # Se o link começa com "t.me", adiciona o esquema "https://"
        if upsell_link.startswith("t.me/"):
...
async def gerar_pix_codigo(update: Update, context: CallbackContext) -> None:
    ...
    if cache['token_type'] == 'PP':
        ...
        add_payment_info(payment_object['payment_id'], chat_id, True)
...
async def pagar_pix(update: Update, context: CallbackContext):
    ...
    if payment_info[4] != "paid":
        ...
        valor_original = float(plano_escolhido['valor'])
        ...
        valor_com_desconto = valor_original- (valor_original*desconto_configurado / 100)
        print(f'valor com desconto {valor_com_desconto}')
        valor_cents = valor_com_desconto

        print(f"Preparando para gerar código PIX com valor descontado: {valor_com_desconto} reais (Desconto: {desconto_configurado}%)")

        try:
            await gerar_pix_codigo_com_valor(update, context, valor_cents)
        except Exception as e:
...
async def gerar_pix_codigo_com_valor(update: Update, context: CallbackContext, valor_cents: float) -> None:
    chat_id = update.effective_chat.id
    plano_escolhido = context.user_data.get('plano_escolhido')
    bot_id = context.user_data.get('bot_id')  # Obtém o bot_id do contexto
    ...
    if cache['token_type'] == 'PP':
        ...
        if not 'pix_code' in payment_object.keys():
            ...
            manager.create_payment(payment_object['payment_id'].lower(), chat_id, json.dumps(plano_escolhido), bot_id)
            add_payment_info(payment_object['payment_id'], chat_id, False)
            pix_code_escaped = escape_markdown_v2(payment_object['pix_code'])
            await context.bot.send_message(chat_id=chat_id, text=f"`{pix_code_escaped}`", parse_mode='MarkdownV2')
...
def run_bot(token, bot_id):
    ...
    try:
        ...
        loop.create_task(agendar_verificacao_expiracao(bot_id, bot_instance))  # Adiciona a verificação de expiração
        ...
        loop.run_until_complete(loop.shutdown_asyncgens())  # Encerra geradores assíncronos, se houver
        loop.close()  # Fecha o loop de eventos

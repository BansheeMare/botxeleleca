...
import requests
default = {
    "config":{'start_text': '💼  Bem-vindo! Defina a mensagem inicial com os comandos /texto, /texto2 ou /midia_inicial.', 'start_text2': '', 'start_media': None, 'button_text': 'CLIQUE AQUI PARA ACESSAR'},
    "recovery":[
            {
                "midia": None,
...
def create_bot(id, token, owner, status="false", config=default['config'], recovery=default["recovery"], plans={}, payment={}, upcell={"text": "", "media": None, "type": None}, group=''):
    if bot_exists(token):
        return False
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO BOTS (id, token, owner, status, config, recovery, plans, payment, users, upcell, 'group') VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (id, token, owner, status, json.dumps(config), json.dumps(recovery), json.dumps(plans), json.dumps(payment), '', json.dumps(upcell), group)
    )
    conn.commit()
    conn.close()
    return
...
def update_bot_upcell(bot_id, upcell):
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    cursor.execute("UPDATE BOTS SET upcell = ? WHERE id = ?", (json.dumps(upcell), bot_id))
    conn.commit()
    conn.close()
...
def initialize_all_registered_bots():
    ...
    for bot in bots:
        # Inicia cada bot em um novo processo
        start_bot(bot[1], bot[0])
        print(f"Bot {bot[0]} iniciado com sucesso.")
